        ...
        _.forEach(thisUsersBetsByOptions, function(value, key) {
          for (var i = 0; i < value.length; i++) {
            ...
            if(bet.finalized === true) { 
              bet.prevResult = bet.balanceAdjustment; 
              bet.prevStatus = bet.status;
            }
            ...
            for (var bi = 0; bi < $scope.gameData.lines.length; bi++) {
              ...
              if(line.lineName === bet.lineName) {
                if (line.lineType === 'vs') {
                  ...
                  var calculatedPoints = currentOption.points + currentOption.optionSpread;
                  calculatedPoints = calculatedPoints.toFixed(2);
                  if (calculatedPoints == otherOption.points) {
                    bet.status = 'Tie';
                  } else {
                    if (calculatedPoints > otherOption.points) {
                      bet.status = 'Winner';
      ...
      _.forEach(finalByUsers, function(value, key) {
        ...
        $http.get("users/sitepoint/"+key).success(function(userresponse){
          ...
          for (var i = 0; i < value.length; i++) {
            ...
            if(bet.finalized === true) {
              if (bet.prevStatus == 'Winner') {
                wins --;
              } else if (bet.prevStatus == 'Loser') {
                loses --;
              }
